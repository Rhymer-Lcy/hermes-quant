# Multi-factor diversification (option B): a modest reversal tilt

After A1 (market-timing) and A2 (position sizing) failed to cut plain value's -33% max
drawdown, B asks whether breadth -- combining value with a diversifying factor -- helps. It
does, for risk-adjusted return: a light reversal tilt raises net Calmar from 0.28 to ~0.32,
above the A2 inverse-vol result (0.30). The gain is in the numerator (return), not the
drawdown -- see the caveat. This result corrects an earlier under-sampled version that
...
Monthly rank-IC correlation across candidate factors (low/negative values indicate diverging bets):
...
The value family (ep/bp/sp) and low-vol are mutually 0.71-0.86 correlated, hence redundant
(this is why the A2 value×low-vol blend added nothing). Reversal is the lone negative.
Treat the matrix as a screen only: it is window-sensitive and lives in rank space,
not return space, so it mis-ranks the diversifiers -- a 10-day reversal shows ≈0 IC-corr yet
combines best (below). The evidence is the combination curve, not this table.
...
whether the benefit is real alpha or merely survives costs):
...
Two results follow. First, a light reversal tilt (value ~80-90%) beats both the value
...
zero cost as well, so it is a genuine timing give-up (value-driven strategies must rotate into
...
Scope note: this null is specific to HS300 (a large-cap, financials-heavy index where
...
The -33% drawdown is not cured. Every variant sits at maxDD -32.8% to -33.8%; the
Calmar gain is entirely CAGR (~+1pp). The systematic, whole-market drawdown is unchanged,
consistent with A1 and A2. The two selection-side levers flagged as "next" were then
...
75/25), which bracket the worst region of the curve, and concluded "no combination
...
sparse sample can land entirely in the wrong region and produce a false negative. The
opposite trap -- selecting the single best point, e.g. rev10 3/1 = 0.35 -- is handled
...
as well -- it had the same standardize-over-the-union survivorship leak A2 surfaced. The ML
