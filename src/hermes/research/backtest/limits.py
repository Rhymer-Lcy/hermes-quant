"""涨跌停 (daily price-limit) no-fill detection for the friction-faithful backtest.
...
ChiNext/STAR, frequent 一字板) where limit no-fill materially affects feasibility.
...
def limit_flags(close: pd.DataFrame, preclose: pd.DataFrame, tol_frac: float = 5e-4) -> pd.DataFrame:
    ...
    `close`/`preclose` are aligned wide panels (前复权 close and previous close, both in the
